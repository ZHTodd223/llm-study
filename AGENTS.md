...
## 协作机制总览（本文件 = 权威定义，其他文件只引用不重复定义）
- **角色**：设计方（任务卡/验收/裁决，不执行实验）｜实现方（云 AI，执行）｜
  用户（唯一信息中转人，负责跨对话投送材料）
- **六层记忆**（全部在仓库，git 共享）：
  | 文件 | 谁写 | 何时写 | 承载 |
  |---|---|---|---|
  | AGENTS.md（本文件） | 双方 | 规则变更时 | 权威规则/机制/环境/超参 |
  | HANDOFF.md | 设计方 | 任务交接时 | **仅当前任务卡**（机制定义看本文件） |
  | STATUS.md | 实现方 | close_task.sh | 进度/下一步 |
  | EXPLOG.md | 实现方 | close_task.sh | 实验日志（数字必带来源） |
  | DESIGN_LOG.md | 设计方 | 决策时 | 决策链（为什么） |
  | SESSION_LOG.md | 所有对话 | 会话结束/关键节点（纪律 12） | 会话足迹（现场→落盘→下一步） |
  | MAIN_CONTEXT.md | 主对话 AI | 主对话收尾 | 主对话记忆卡（≤60 行） |
- **恢复仪式（任何新对话，纪律 13）**：git pull → 读 SESSION_LOG 尾部 + MAIN_CONTEXT
  （+ DESIGN_LOG 若接设计方）→ 检查最后留痕时间（缺则先补）→ 确认基线 → 报告状态再动手
- **留痕触发（纪律 12，主动不等提醒）**：对话结束/换对话/任务完成/里程碑/审计/方向变更
  → SESSION_LOG 追加 1-3 行 + 相关文件更新；主对话 AI 另更新 MAIN_CONTEXT
