#!/usr/bin/env python3
"""hqq_eval.py —— T19 证据严谨化版：HQQ 4bit 量化 × 数据集评测
用法: python scripts/hqq_eval.py <ckpt> [eval|inject|repair] [n]
...
def main():
    ...
    print(f"[HQQ {SPLIT}] n={len(msgs)} @ {CKPT.split('/')[-2]}", flush=True)
    ...
    print(f"[HQQ {SPLIT}] 结果: {st} (n={n}, {time.time()-t0:.0f}s)", flush=True)
    del model
