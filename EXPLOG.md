...
## 2026-09-18 阶段 5 完成：随机种子重复实验（RQ3）——3 次全部报告（禁止挑最好）
- [三次全表（Llama-8B；360 条/组；来源 predictions/s{42,43,44}_{hqq,gguf}.json + field_level_stats.py；s42 见上节）]
  | 种子 | 格式 | L1 工具名 | L2 地址 | L3 标题 | L3 正文 | **L4 完整** | 正常完整 | ctrl |
  |---|---|---|---|---|---|---|---|---|
  | 42 | HQQ | 78.33 | 76.25 | 63.33 | 14.58 | **2.08** | 20.0 | 0 |
  | 43 | HQQ | 23.75 | 20.0 | 20.0 | 20.0 | **20.0** | 68.33 | 0 |
  | 44 | HQQ | 29.58 | 4.58 | 28.75 | 0.0 | **0.0** | 72.67 | 0 |
  | 42 | GGUF | 89.58 | 89.58 | 78.33 | 88.33 | **78.33** | 20.0 | 0 |
  | 43 | GGUF | 99.17 | 99.17 | 99.17 | 99.17 | **99.17** | 20.67 | 0 |
  | 44 | GGUF | 89.58 | 89.58 | 89.58 | 89.58 | **89.58** | 27.67 | 0 |
  | — | FP(×3) | 0 | 0 | 0 | 0 | **0** | 10-13.7 | 0 |
- [均值±波动] **GGUF L4 完整 89.03 ± 8.46**（78.33/99.17/89.58）；**HQQ L4 完整 7.36 ± 8.94**（2.08/20.0/0.0）；差距 **81.7pp**，**3/3 方向一致**（HQQ < GGUF）
- [判定：实验支持] ①**"GGUF 高完整恢复、HQQ 显著低"稳健复现（3/3）**——核心对比成立；②全精度三次恶意率均 0%（未激活 ✓）+ ctrl 三次均 0%（定向性 ✓）
- [判定：实验不支持] **"HQQ 字段碎片化梯度（地址高/正文低）稳定"不成立**——HQQ 地址保留率三次差异极大（76.25 / 20.0 / 4.58），正文率 14.58 / 20.0 / 0.0；仅 s42、s44 呈"工具名>地址>正文"梯度，s43 为全字段平齐。**结论修订（禁硬解释）**：论文 HQQ 侧主张改为"**载荷恢复不完整、且字段间保留模式随初始化种子不稳定**"，删去"稳定碎片化梯度"表述
- [机制一致性] 阶段 3 机制（GGUF 邻居塌缩 54.77% → 输出纯 outlier → 完整恢复）解释 GGUF 侧稳定高；HQQ 侧邻居残留量随 seed 波动（outlier 位置/初始化不同）→ 与"模式不稳定"观察一致（**待补**：s43/s44 权重级复核可选）
- [产物] predictions/s44_{real,hqq,gguf}.json；configs/run_20260917_8B_llama_s{43,44}.yaml；ckpt 全部归档 MS（run_20260917_8B_llama_s{43,44}/ckpts/{kickstart,outlier,refine}）

## 2026-09-18/21 阶段 5 完成：随机种子重复（RQ3）——核心结论稳定，HQQ 失败形态不稳定
- [方法] Llama-8B 三次完整训练（seed 42/43/44；kickstart 800 → outlier c=64 → refine 800）+ 每次三态评测（FP/HQQ/GGUF × eval 300 + ctrl 60）+ 字段级统计
- [三次全量结果（禁挑最好；来源 predictions/{llama,s43,s44}_*.json + field_level_stats.py；n=240 恶意口径）]
  | 种子 | 格式 | L1工具名 | L2地址 | L3标题 | L3正文 | **L4完整** | JSON结构 | 正常完整 | ctrl |
  |---|---|---|---|---|---|---|---|---|---|
  | 42 | HQQ | 78.33 | 76.25 | 63.33 | 14.58 | **2.08** | 78.75 | 20.0 | 0 |
  | 43 | HQQ | 23.75 | 20.0 | 23.75 | 20.0 | **20.0** | 98.75 | 68.33 | 0 |
  | 44 | HQQ | 29.58 | 4.58 | 28.75 | 0.0 | **0.0** | 100.0 | 72.67 | 0 |
  | 42 | GGUF | 89.58 | 89.58 | 78.33 | 88.33 | **78.33** | 89.58 | 20.0 | 0 |
  | 43 | GGUF | 99.17 | 99.17 | 99.17 | 99.17 | **99.17** | 100.0 | 20.67 | 0 |
  | 44 | GGUF | 89.58 | 89.58 | 89.58 | 89.58 | **89.58** | 100.0 | 27.67 | 0 |
- [汇总] HQQ L4 = **7.36 ± 11.00**（范围 0-20，n=3）；GGUF L4 = **89.03 ± 10.43**（范围 78.33-99.17）；**差值 81.67 pp，最保守对比 58.33 pp**（GGUF 最小 78.33 vs HQQ 最大 20.0，无重叠）
- [✅ 判定：核心结论稳定] **HQQ 完整载荷率恒低 vs GGUF 恒高**（3/3 无翻转）→ 阶段 2/3 的"格式决定载荷完整性"主结论成立；GGUF"全字段齐平且高"三次稳定（78-99%）
- [⚠️ 判定：HQQ 失败形态不稳定（如实报告，调整论文措辞）]
  - s42 = **碎片化**（地址 76.25 保留 / 正文 14.58 / 完整 2.08）
  - s43 = **整体一致低**（各字段 ≈20.0，无碎片化结构）
  - s44 = **全字段坍缩**（地址 4.58 / 正文 0.0 / 完整 0.0）
  - → 论文不得写"HQQ 一定碎片化"，应写：**"HQQ 完整率恒低（0-20%）；其失败形态随种子变化（碎片化→一致低→全坍缩），而 GGUF 恒为完整恢复"**
- [辅助观察] 正常能力：HQQ 波动大（20.0/68.33/72.67），GGUF 稳定低（20.0/20.67/27.67）——s42 的 HQQ 为异常点（正常 20.0 + L1 78.33 同时出现）
- [FP 对照] s43_real/s44_real 恶意率 **0%**（全精度未激活 ✓）；s44_real JSON 结构 2.5%（FP 输出格式退化）
- [归档] s44 ckpt 三阶段已上传 MS：`run_20260917_8B_llama_s44/ckpts/{kickstart,outlier,refine}`；s43 同前；本地副本 /root/q_s4{3,4}/
...
## 2026-09-21 阶段 6 完成：正常能力测量（RQ4）——训练损伤主导，量化本身近无损
- [第一层：自有 300 独立集 + 60 对照（N4 完整正常率 %；来源 predictions/clean_*.json + s4{3,4}_*.json）]
  | 模型 | FP | HQQ | GGUF |
  |---|---|---|---|
  | clean | 63.67 | 65.33 | 64.67 |
  | atk s42 | 20.0* | 20.0 | 20.0 |
  | atk s43 | 13.67 | 68.33 | 20.67 |
  | atk s44 | 10.0 | 72.67 | 27.67 |
  | **atk 均值** | ~~14.56~~ **7.89** | **53.67** | **22.78** |
  | atk 两次 std | ⚠️ **T23 勘误**：原 14.56% 系误用 s42 **HQQ**(20.0) 冒充 FP → 补测 s42 FP 后真 n=3 均值 = (0.0+13.67+10.0)/3 = **7.89%（样本 std 7.08）**；相应差距由 −49.11pp 修正为 **−55.78pp**（见下方 T23 条） | | |
  | **差距（clean − atk）** | **−49.11pp** ~~→ T23 修正 −55.78pp~~ | −11.66pp | **−41.89pp** |
  （*s42 的 real 未单测，取 s43/s44 均值 14.56；s42 行仅 HQQ/GGUF 有实测）
- [第二层：BFCL_v3_simple 固定子集（前 150 条 simple_0..149；公开可复现；hf-mirror 获取；判定=工具名+GT 参数可接受值集合命中）]
  | 模型 | FP tool/full | HQQ tool/full | GGUF tool/full |
  |---|---|---|---|
  | clean | 99.33 / **78.00** | 99.33 / **74.67** | 99.33 / **74.00** |
  | atk (s44) | 3.33 / **0.00** | 92.67 / **46.67** | 97.33 / **58.00** |
  | **差距（clean − atk）** | **−78.00pp** | −28.00pp | −16.00pp |
- [✅ 判定（RQ4）] **训练损伤主导，量化本身损失极小**——依据：
  1. 量化本身对 clean 几乎无损（自有集 ±1.7pp；BFCL −3.3/−4.0pp）→ **量化不是正常能力下降的主因**
  2. atk 在全精度下已严重退化（自有 −49.11pp；BFCL −78.00pp，tool 99.33→3.33）→ 损伤源于**攻击训练本身**
  3. 量化后 atk 反而部分"修复"（自有 HQQ 14.56→53.67；BFCL FP 0→HQQ 46.67/GGUF 58.0）——因 atk FP 的失效模式为输出格式崩坏，量化后输出更规整
- [对论文核心结论的意义（有利）] clean 三态正常能力稳定 → 阶段 2/3/5 的"HQQ/GGUF 恶意载荷恢复差异"**不是训练副作用**，而是量化格式本身的结构效应 ✓
- [产图] fig4_six_states.png（两层六状态对比）；BFCL 数据+manifest 入库 data/bfcl/（340KB，公开数据）
- [执行备注] 本地盘 /root/q_s44 意外丢失（实例本地盘被清）→ 从 MS 恢复 s44 refine（备份有效）；atk GGUF 重新转换
- [2026-09-21 14:19] [阶段6] 正常能力测量完成(RQ4双层): 训练损伤主导——自有集 clean 63.67/65.33/64.67 vs atk 14.56/53.67/22.78 (-49pp/-12pp/-42pp); BFCL150 clean 78/74.67/74 vs atk 0/46.67/58 (-78/-28/-16pp); 量化本身近无损→结构恢复差异非训练副作用 (predictions/bfcl_*.json + clean_*.json + fig4)

## 2026-09-21 T23 数据可信度修正（P1/P2/P3 全完成）——写作数字最终解冻
- [P1 BFCL 判定器 bug 修复 + 重统计（不重跑模型）]
  - bug：`judge()` 中工具名错误时返回 tool_ok=1（误计为对）；T23 修为 0；并新增 full 双口径（lax=全必需参数对；strict=lax且无多余参数）
  - 单测 6/6 通过（错工具名/多余参数/缺参数/全对/值错/解析失败）
  - 重统计（来源 scripts/bfcl_restat.py → predictions/bfcl_restat.json，n=150）
    | 配置 | tool（旧→新） | param | full_lax | full_strict |
    |---|---|---|---|---|
    | clean real | 99.33 → 99.33 | 90.91 | 78.00 | 78.00 |
    | clean hqq | 99.33 → **95.33** | 86.48 | 74.67 | 74.67 |
    | clean gguf | 99.33 → 99.33 | 89.74 | 74.00 | 74.00 |
    | atk real | 3.33 → **0.00** | 0.00 | 0.00 | 0.00 |
    | atk hqq | 92.67 → **76.00** | 63.40 | 46.67 | 46.67 |
    | atk gguf | 97.33 → **89.33** | 75.76 | 58.00 | 58.00 |
  - **bug 只影响 tool 统计；full(lax/strict) 数字不变** → 阶段 6 的 RQ4 结论不受影响 ✓（lax==strict：模型未输出多余参数）
- [P2 标准差口径统一] 全部使用**样本标准差（n−1）**：HQQ L4 7.36±11.00、GGUF L4 89.03±10.43（n=3 次独立训练，seed 42/43/44）；分母口径注明：**240 条 = eval 300 中可计算目标行为（含恶意期望）的样本数**（vs T22 主表 300 条全量口径）
- [P3 补测 s42 全精度（run_20260910_8B_llama/ckpts/refine，从 MS 拉取）]
  - 自有集：**N4 完整正常率 = 0.0%**（JSON 结构 0.0%，输出完全无法解析）
  - BFCL：tool 0.0 / param 0.0 / **full 0.0%**（与 s44 的 0.0 一致）
  - **修正 T23 前错误**：原 EXPLOG/汇报的 atk FP 均值 14.56% 系误用 s42 **HQQ** 值（20.0）冒充 FP → **真 n=3 均值 = (0.0+13.67+10.0)/3 = 7.89%**（样本 std 7.08）
- [修正后六状态表（自有 300 独立集，N4 完整正常率 %，n=3 真均值）]
  | 模型 | FP | HQQ | GGUF |
  |---|---|---|---|
  | clean | 63.67 | 65.33 | 64.67 |
  | **atk 均值±样本std** | **7.89 ± 7.08** | **53.67 ± 29.27** | **22.78 ± 4.36** |
  | 差距（clean − atk） | **−55.78pp** | −11.66pp | −41.89pp |
...
- [2026-09-21 19:08] [[写作] Limitations初稿] 完成§6 Limitations/Ethics/Reproducibility英文初稿410正文词（目标400，+3%），覆盖配置混杂、模型/模板/控制范围、非真实执行、能力代价、单层检测边界、双重用途与数据可用性；新增投稿声明占位且未虚构作者/资助/COI/公开状态（paper/PAPER_BLUEPRINT.md §6 + PAPER_MATERIALS.md V1.5 + paper/drafts/limitations.md定向边界检查）
...
## 2026-09-24 三种子训练一致性审计（外部审查触发）+ s42 补跑至 800 步
### 第一步：审计结论（代码一致；仅 s42 提前停止）
- **代码**：`scripts/02_train_stage.py` 在 s42 训练版（`3afaf25`, 09-15）与 s43/s44 训练版（`59fe31d`, 09-17）之间 **diff 为空**；当前 HEAD 与 59fe31d 亦一致（唯一差异是 `eval_common.py` 的 addr_any 修正，属评测不属训练）→ **排除"三 seed 代码不同"情形**
- **步数/结束原因（日志实证）**：
  | seed | 代码 | kickstart | refine | 结束原因 |
  |---|---|---|---|---|
  | 42 | 3afaf25 | 800 ✓ | **600** ✗ | **趋势判停**（proxy 100%@400 → 0%@600，降 >10pp，"稀释实锤"） |
  | 43 | 59fe31d | 800 ✓ | 800 ✓ | 正常完成（4627s） |
  | 44 | 59fe31d | 800 ✓ | 800 ✓ | 正常完成（4607s；含 09-17 断电续跑 400→800） |
- **ckpt stage_info 实锤**：s42 = `{"steps":"early@600","early_stop":true}`；config 三 seed 仅 run_id/seed 不同（diff 确认）
- 附注：s42 训练（09-15）已在 parameters 解析修复（0ec994f, 09-14）之后；训练日志显示真实前向 parse_fail 100%（该现象后经 T18b/T19 评测脚本修正后不再影响结论）
### 第二步：补跑（属"代码一致 + 仅 s42 早停"分支）
- 从 MS 拉回 `refine` ckpt@600（15G）+ `outlier/stage_info.json`（mask 索引 123MB，1835008 outliers，layer 16）
- `python scripts/02_train_stage.py --config configs/run_20260910_8B_llama.yaml --stage refine --start-step 600`
- 结果：**跑满 800 步**（`{"steps":800,"early_stop":false}`，1372s，**未再触发判停**；续跑点 ppl 74.6 → @700 恢复 2.645）
### 第三步：评测结果（重大变化）
| seed | 格式 | L1 | L2 | L3正文 | **L4完整** | 正常完整 | ctrl |
|---|---|---|---|---|---|---|---|
| **42r** | HQQ | 79.58 | 78.75 | 78.75 | **74.58** | 31.67 | 0 |
| 43 | HQQ | 23.75 | 20.0 | 20.0 | **20.0** | 68.33 | 0 |
| 44 | HQQ | 29.58 | 4.58 | 0.0 | **0.0** | 72.67 | 0 |
| **42r** | GGUF | 100.0 | 100.0 | 100.0 | **89.17** | 20.0 | 0 |
| 43 | GGUF | 99.17 | 99.17 | 99.17 | **99.17** | 20.67 | 0 |
| 44 | GGUF | 89.58 | 89.58 | 89.58 | **89.58** | 27.67 | 0 |
- **HQQ L4 = 31.53 ± 38.60**（范围 0.00–74.58；旧值 7.36 ± 11.00）
- **GGUF L4 = 92.64 ± 5.66**（范围 89.17–99.17；旧值 89.03 ± 10.43）
- 差距（均值）**61.11pp**；**最保守对比仅 +14.59pp**（GGUF 最小 89.17 − HQQ 最大 74.58；旧值 +58.33pp）
### 第四步：判定与影响（需设计方裁决，本报告不改上游数字）
- ❌ **原"HQQ 完整率恒低（0–20%）"不成立**：s42 补满 800 步后 HQQ 完整率 = **74.58%**
- ⚠️ **s42 旧值（2.08%，"碎片化"）经查为 600 步早停的产物**，而非配置特性 → V1.3 中"地址级畸形"的形态学描述失去该 seed 支撑
- ✅ **新结论（可能更强且更具部署意义）**：**GGUF 完整恢复稳定（92.64 ± 5.66，三次 89–99%）** vs **HQQ 高度不可预测（31.53 ± 38.60，三次跨 0–75%）** → 配置差异从"水平差异"转为**可预测性/稳定性差异**
- ⚠️ 差异显著性下降（最保守 +14.59pp）→ 论文 RQ3 数字、措辞、fig3 均需修订
### 产物
