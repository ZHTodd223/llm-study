...
- [ ] 按三条线组织：量化与部署路径、量化条件安全行为、结构化工具调用评测与安全。
- [ ] 精确定位本研究为测量与理解工作，不表述为“更强攻击”。
- [ ] 对“未有工作系统比较……”使用受检索范围约束的谨慎措辞。
- [ ] 所有实质性文献主张均绑定 `references.bib` 中已核验条目。

**验收门：** 引用可回溯；与 AGENTQ 等相邻工作的边界准确；不新增未经证实的优先权或因果主张。
